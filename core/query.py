...
class BooleanQueryParser:
    ...
    Требования к индексу:
      - get_postings(term, *, field=None) -> Dict[doc_id, List[int]]  # позиции терма
      - documents: Dict[doc_id, Any]                                  # для all_docs
      - doc_id сравнимы и сортируются стандартно (строки/числа).
    """
    ...
    def parse(self, query: str) -> List[str]:
        ...
        tokens = self._normalize_ops(tokens)
        tokens = self._insert_implicit_or(tokens)
    ...
    def _tokenize(self, s: str) -> List[str]:
        ...
        buf: List[str] = [] # текущий буфер» символов для строящегося токена
        i = 0
        ...
        while i < len(s):
            ...
            if ch == '"':
                buf.append(ch)
                i += 1
                # собрать до следующей кавычки (включительно)
                while i < len(s):
                    buf.append(s[i])
                    if s[i] == '"':
                        i += 1
    ...
    def _insert_implicit_or(self, tokens: List[str]) -> List[str]:
    ...
    def _eval(self, node: "_Node") -> List[str]:
        ...
        if isinstance(node, self._Prox):
            # Используем методы из объединенного индекса
            ...
            # Получаем позиции через объединенный индекс
    ...
    # ========= Работа с фразами (координатный индекс, k-way) =========
    ...
    def _phrase_positions(self, terms: List[str], field: Optional[str]) -> Dict[str, List[int]]:
        ...
        for doc in common_docs:
            chains = sorted(postings_list[0][doc])  # позиции первого слова
            for k in range(1, len(terms)):
                nxt = sorted(postings_list[k][doc])
                chains = self._adjacent_positions(chains, nxt)  # оставляет позиции nxt, где nxt == chains+1
                if not chains:
                    break
            if chains:
                result[doc] = chains
    ...
    # ========= Многопутевые (k-way) операции над отсортированными docID =========
    ...
    def _intersect_many_kway(self, lists: Sequence[List[str]]) -> List[str]:
        ...
        if len(lists) == 1:
            return lists[0]
        lsts = list(lists)
        # Сортируем по длине: выгоднее выравниваться по коротким головам.
        lsts.sort(key=len)
        k = len(lsts)
        idx = [0] * k
        ...
        # Инициализация target = максимум текущих голов
        def current_heads():
            return [lsts[i][idx[i]] for i in range(k)]

        # Быстрый выход: если хоть один пустой — уже вернули выше
        while True:
            # проверка конца
            for i in range(k):
                if idx[i] >= len(lsts[i]):
            ...
            # продвигаем каждую голову до target (или дальше)
            all_equal = True
            for i in range(k):
                if lsts[i][idx[i]] < target:
                    ...
                    if idx[i] >= len(lsts[i]):
                        return out
                # после продвижения сверяем равенство
                if lsts[i][idx[i]] != target:
                    all_equal = False
            ...
            if all_equal:
                ...
                # новый цикл с обновлёнными головами
                continue
    ...
    def _union_many_heap(self, lists: Sequence[List[str]]) -> List[str]:
        ...
        heap: List[Tuple[str, int]] = []  # (doc_id, idx списка)
    ...
    def _split_phrase_terms(phrase: str) -> List[str]:
        # должен совпадать с токенизацией индекса!
